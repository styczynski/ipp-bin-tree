...
function check_out_script {
  ...
  if [[ $ok != 'true' ]]; then
    ...
    err_message=$(echo -en "$err_message" | sed "s/^/ $B_ERR\|$E_ERR  /g")
    if [[ $flag_extreamely_minimalistic = 'false' ]]; then
      printf  "%-35s  %s\n" "${B_DEBUG}[$file_index/$file_count]${E_DEBUG}  $input_file" "${B_ERR}[ERR] Invalid tester answer${E_ERR}"
    else
      printf  "${B_ERR}$input_file${E_ERR}\n"
    fi
    if [[ $flag_very_minimal = 'false' ]]; then
...
function check_out_script_err {
  ...
  if [[ $ok != 'true' ]]; then
    ...
    err_message=$(echo -en "$err_message" | sed "s/^/ $B_ERR\|$E_ERR  /g")
    if [[ $flag_extreamely_minimalistic = 'false' ]]; then
      printf  "%-35s  %s\n" "${B_DEBUG}[$file_index/$file_count]${E_DEBUG}  $err_path" "${B_ERR}[ERR] Invalid tester answer for stderr${E_ERR}"
    else
      printf  "${B_ERR}$err_path${E_ERR}\n"
    fi
    if [[ $flag_very_minimal = 'false' ]]; then
...
function check_testing_script {
  ...
  if [[ $test_not_exists = 'true' ]]; then
      printf  "%-35s  %s\n" "${B_DEBUG}[$file_index/$file_count]${E_DEBUG}  $input_file" "${B_ERR}[ERR] Testing script does not exists or is invalid command (--tscript). Abort.${E_ERR}"
...
function check_testing_script_err {
  ...
  if [[ $test_not_exists = 'true' ]]; then
      printf  "%-35s  %s\n" "${B_DEBUG}[$file_index/$file_count]${E_DEBUG}  $err_path" "${B_ERR}[ERR] Testing script does not exists or is invalid command (--tscript-err). Abort.${E_ERR}"
